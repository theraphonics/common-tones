...
(progn
  ...
  (defun with-augmented-environment-internal (env functions macros)
    ((lambda (new-env) (vector (svref env 0) new-env)) 
      (let* ((new-env (make-array (+ (* (+ (length functions) (length macros)) 2) 1)))
        ...
        (setf (svref new-env i) (svref env 1)
        )
        new-env
  ) ) )
  (defun environment-function (env fn)
    (setq env (svref env 1)) 
...
(defun variable-globally-special-p (symbol)
  ...
  #+(or openmcl :CORAL)                      (ccl::proclaimed-special-p symbol)
  #-(or openmcl Genera Cloe-Runtime Lucid Xerox Excl KCL IBCL (and dec vax common) :CMU HP-HPLabs
...
(defun get-walker-template (x)
  (cond ((symbolp x)
	...
	((and (listp x) (eq (car x) 'lambda))
	 '(lambda repeat (eval)))
        #+(or cmu CLISP) ; #+setf is defined later, in macros.lsp
...
(defconstant *toplevel-environment* 
  #-clisp nil
  #+clisp (vector nil nil))
...
(defun WALK-FORM (form
		  &optional (environment *toplevel-environment*)
			    (walk-function
	  ...
	  (let* ((fn (car newform))
		...
		(multiple-value-bind
		  ...
		  (cond
		   ...
		   ((and (symbolp fn)
			 (not (fboundp fn))
			 #-(or allegro-v7.0 allegro-v8.0 allegro-v8.1 allegro-v8.2) (special-form-p fn)
			 #+(or allegro-v7.0 allegro-v8.0 allegro-v8.1 allegro-v8.2) (special-operator-p fn)
			 )
		    (error
...
(defun take-it-out-for-a-test-walk-1 (form)
  ...
  (let ((copy-of-form (copy-tree form))
	(result (walk-form form *toplevel-environment*
		  #'(lambda (x y env)
