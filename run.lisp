...
(defun run-type->class (type) ;first (second?) step in gen-unload process -- get type, dispatch to method
  (cond
   ...
   ((= type +string+) "1")
;   ((= type +frame+) (make-instance 'frame))
;   ((= type +mixer+) (make-instance 'mixer))
   ((= type +oscil+) (make-instance 'oscil))
   ((= type +ncos+) (make-instance 'ncos))
   ((= type +nrxycos+) (make-instance 'nrxycos))
   ((= type +nsin+) (make-instance 'nsin))
   ((= type +nrxysin+) (make-instance 'nrxysin))
   ((= type +ssb-am+) (make-instance 'ssb-am))
   ((= type +rand+) (make-instance 'rand))
   ((= type +rand-interp+) (make-instance 'rand-interp))
   ((= type +table-lookup+) (make-instance 'table-lookup))
   ((= type +square-wave+) (make-instance 'square-wave))
   ((= type +pulse-train+) (make-instance 'pulse-train))
   ((= type +sawtooth-wave+) (make-instance 'sawtooth-wave))
   ((= type +triangle-wave+) (make-instance 'triangle-wave))
   ((= type +asymmetric-fm+) (make-instance 'asymmetric-fm))
   ((= type +wave-train+) (make-instance 'wave-train))
   ((= type +one-pole+) (make-instance 'one-pole))
   ((= type +two-pole+) (make-instance 'two-pole))
   ((= type +one-zero+) (make-instance 'one-zero))
   ((= type +two-zero+) (make-instance 'two-zero))
   ((= type +delay+) (make-instance 'delay))
   ((= type +tap+) (make-instance 'tap))
   ((= type +comb+) (make-instance 'comb))
   ((= type +filtered-comb+) (make-instance 'filtered-comb))
   ((= type +notch+) (make-instance 'notch))
   ((= type +all-pass+) (make-instance 'all-pass))
   ((= type +moving-average+) (make-instance 'moving-average))
   ((= type +filter+) (make-instance 'filter))
   ((= type +fir-filter+) (make-instance 'fir-filter))
   ((= type +iir-filter+) (make-instance 'iir-filter))
   ((= type +env+) (make-instance 'seg))
   ((= type +locsig+) (make-instance 'locsig))
   ((= type +move-sound+) (make-instance 'move-sound))
   ((= type +src+) (make-instance 'src))
   ((= type +granulate+) (make-instance 'granulate))
   ((= type +readin+) (make-instance 'readin))
   ((= type +convolve+) (make-instance 'convolve))
   ((= type +polyshape+) (make-instance 'polyshape))
   ((= type +polywave+) (make-instance 'polywave))
   ((= type +firmant+) (make-instance 'firmant))
   ((= type +formant+) (make-instance 'formant))
   ((= type +sample2file+) (make-instance 'sample->file))
   ((= type +frample2file+) (make-instance 'frample->file))
   ((= type +file2sample+) (make-instance 'file->sample))
   ((= type +file2frample+) (make-instance 'file->frample))
   ((= type +phase-vocoder+) (make-instance 'phase-vocoder))
...
(defmethod gen-make ((gen move-sound) stream result iloc indent ctr)
  ...
  (gen-make (make-instance 'delay) stream "doppler_delay" "iloc" indent ctr)

  (format stream "~Ailoc = CLM_ILOC(~A) + 8;~%" indent iloc)
  (gen-make (make-instance 'seg) stream "doppler_env" "iloc" indent ctr)

  (format stream "~Ailoc = CLM_ILOC(~A) + 10;~%" indent iloc)
  (gen-make (make-instance 'seg) stream "rev_env" "iloc" indent ctr)

  (format stream "~Ailoc = CLM_ILOC(~A) + 12;~%" indent iloc)
  (move-sound-array-make (make-instance 'delay) stream "out_delays" indent)

  (format stream "~Ailoc = CLM_ILOC(~A) + 14;~%" indent iloc)
  (move-sound-array-make (make-instance 'seg) stream "out_envs" indent)

  (format stream "~Ailoc = CLM_ILOC(~A) + 16;~%" indent iloc)
  (move-sound-array-make (make-instance 'seg) stream "rev_envs" indent)
